...
namespace TinyPG.CodeGenerators.CSharp
{
    public class ScannerGenerator : BaseGenerator, ICodeGenerator
    {
        ...
        public string Generate(Grammar Grammar, bool Debug)
        {
            ...
            {
                if (t.Attributes.ContainsKey("Style"))
                {
                    int len = t.Attributes["Style"].Length;
                    if (len == 1)
                    {
                        if (t.Attributes["Style"][0] is string)
                        {
                            var style = Convert.ToString(t.Attributes["Style"][0]);
                            styles.Append($"\r\n{{TokenType.{t.Name}, {style}}},");
                        }
                    }
                }
            }
            ...
            scanner = scanner.Replace(@"<%TokenStyles%>", styles.ToString());
